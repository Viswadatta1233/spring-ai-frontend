...
const buildQueryUrl = (endpoint, queryString) => {
  const query = typeof queryString === "string" && queryString !== "undefined" ? queryString.trim() : "";
...
export const fetchProducts = (queryString) => async (dispatch) => {
    try {
        ...
        const { data } = await api.get(buildQueryUrl(`/public/products`, queryString));
        dispatch({
            type: "FETCH_PRODUCTS",
...
export const semanticSearchProducts =
    (query, { topK = 12, minPrice, maxPrice } = {}) => async (dispatch) => {
    try {
        dispatch({ type: "IS_FETCHING" });
        const params = new URLSearchParams({ query, topK: String(topK) });
        if (minPrice) params.set("minPrice", String(minPrice));
        if (maxPrice) params.set("maxPrice", String(maxPrice));
        const { data } = await api.get(`/public/products/semantic-search?${params.toString()}`);
        dispatch({
            type: "FETCH_PRODUCTS",
            payload: data.content,
            pageNumber: data.pageNumber,
        ...
        dispatch({
            type: "IS_ERROR",
            payload: error?.response?.data?.message || "Smart search failed. Please try again.",
        });
    }
};
...
export const stripePaymentConfirmation 
    = (sendData, setErrorMesssage, setLoadng, toast, navigate) => async (dispatch, getState) => {
        try {
...
export const getOrdersForDashboard = (queryString, isAdmin) => async (dispatch) => {
    try {
        ...
        const { data } = await api.get(buildQueryUrl(endpoint, queryString));
        dispatch({
...
export const getAllCategoriesDashboard = (queryString) => async (dispatch) => {
  dispatch({ type: "CATEGORY_LOADER" });
  try {
    const { data } = await api.get(buildQueryUrl(`/public/categories`, queryString));
    dispatch({
      type: "FETCH_CATEGORIES",
      payload: data["content"],
      pageNumber: data["pageNumber"],
...
export const deleteCategoryDashboardAction =
  ...
  (queryString) => async (dispatch, getState) => {
    ...
    try {
      dispatch({ type: "IS_FETCHING" });
      const { data } = await api.get(buildQueryUrl(`/auth/sellers`, queryString));
      dispatch({
        type: "GET_SELLERS",
        payload: data["content"],
        pageNumber: data["pageNumber"],
